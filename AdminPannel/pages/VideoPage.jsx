...
import {
  adminDoctorDetailsGet,
  adminDoctorDetailsUpdate,
...
const VideoFetch = () => {
  ...
  const getDoctorDetails = async () => {
    const responseJson = await adminDoctorDetailsGet(id);
    if (responseJson.data.status) {
      setVideo(responseJson.data.data.video);
    }
  };
  ...
  const handlePost = async () => {
    if (videoInfo.title && videoInfo.link) {
      ...
      tempvideo.push(videoInfo);
      const responseJson = await adminDoctorDetailsUpdate({ video: tempvideo });
      if (responseJson.data.status) {
  ...
  const deleteVideo = async (video) => {
    const responseJson = await adminDoctorDetailsUpdate({ video });
    if (responseJson.data.status) {
      toast.success(responseJson.data.message);
      // getDoctorDetails();
    } else {
  ...
  return (
    <React.Fragment>
      <ThemeProvider theme={theme}>
        ...
        <div className="mt-2">
          <UploadButton onClick={handlePost}>
