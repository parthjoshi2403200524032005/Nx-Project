...
const UserPage = () => {
  ...
  const forGetData = async (rowsPerPage, page, searchterm) => {
    try {
      const res = await adminGetAllUsers(rowsPerPage, page, searchterm);
      if (res?.data?.data) {
  ...
  const handleChange = async (e, id) => {
    ...
    const responseJson = await adminUpdateUser(requestBody, tempdata[id]._id);
  ...
  const handleDeleteUser = async (id) => {
    const responseJson = await adminDeleteUser(id);
    if (responseJson.data.status) {
      toast.success(responseJson.data.message);
    } else {
      toast.error(responseJson.data.message);
  ...
  return (
    <React.Fragment>
      <ThemeProvider theme={theme}>
        ...
        >
          <MenuItem onClick={() => navigate(`/admin/dashboard/user/${editid}`)}>
            <EditIcon />
            Edit
          </MenuItem>
