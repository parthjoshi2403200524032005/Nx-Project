...
import {
  getSpecificDoctor,
  adminTreatmentPost,
  adminTreatmentDetailsUpdate,
  adminTreamentDetailsDelete,
} from "../../Service/Services";
import DetailCard from "../../DoctorPannel/DoctorComponents/DetailCard";
...
const TreatmentPage = () => {
  ...
  const forAddPackage = (e) => {
    ...
    if (
      ...
      newpackage.treatmentplace !== "" && newpackage.about !== '' &&
      parseInt(newpackage.maxprice) > 0 &&
      parseInt(newpackage.minprice) > 0
    ) {
  ...
  const getDoctorDetails = async () => {
    const responseJson = await getSpecificDoctor(id);
  ...
  const handleAddPackage = async () => {
    const responseJson = await adminTreatmentPost(id, newpackage);
    if (responseJson?.data.status) {
  ...
  const handleUpdatePackage = async () => {
    const responseJson = await adminTreatmentDetailsUpdate(newpackage, edit, id);
    if (responseJson?.data.status) {
  ...
  const handleUpdateDelete = async (hid) => {
    const responseJson = await adminTreamentDetailsDelete(hid, id);
    if (responseJson?.data?.status) {
