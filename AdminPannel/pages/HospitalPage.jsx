...
import {
  adminHospitalDetailsDelete,
  adminHospitalDetailsUpdate,
  adminHospitalsPost,
  aws_url,
  getSpecificDoctor,
...
const HospitalPage = () => {
    ...
    try {
      if (
        ...
        if (edit) {
          ...
          await adminHospitalsPost(id, newhospital);
        }
  ...
  const handleHospitalUpdate = async () => {
    const responseJson = await adminHospitalDetailsUpdate(
      newhospital,
      edit,
      id
    );
  ...
  const forGetHospitals = async () => {
    const response = await getSpecificDoctor(id);
  ...
  const forDeleteHospital = async (hid) => {
    // setHospital(hospital.filter((_, xd) => xd !== index));
    console.log(hid);
    const response = await adminHospitalDetailsDelete(hid, id);
    if (response?.data.status) {
